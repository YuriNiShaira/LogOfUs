...
const YearCard: React.FC<YearCardProps> = ({ year, onClick }) => {
  ...
  // Mobile: No hover animation, just static card
  if (isMobile) {
    return (
      <div
      ...
      >
        ...
        <style>{`
          .journal-grain {
            background-image: url("data:image/svg+xml,%3Csvg viewBox=%220 0 200 200%22 xmlns=%22http://www.w3.org/2000/svg%22%3E%3Cfilter id=%22noiseFilter%22%3E%3CfeTurbulence type=%22fractalNoise%22 baseFrequency=%220.85%22 numOctaves=%223%22 stitchTiles=%22stitch%22/%3E%3C/filter%3E%3Crect width=%22100%25%22 height=%22100%25%22 filter=%22url(%23noiseFilter%22)%22/%3E%3C/svg%3E");
          }
        `}</style>

        {/* Stacked Back Page */}
        <div className={`absolute inset-0 rounded-2xl border origin-bottom-left rotate-2 translate-x-2 translate-y-2 shadow-sm ${layerBg} ${borderColor}`}>
        ...
        {/* Stacked Middle Page */}
        <div className={`absolute inset-0 rounded-2xl border origin-bottom-left -rotate-1 translate-x-0.5 shadow-sm ${paperBg} ${borderColor}`}>
          <div className="absolute inset-0 opacity-[0.04] journal-grain mix-blend-multiply dark:mix-blend-overlay rounded-2xl pointer-events-none" />
        </div>

        {/* Main Card */}
        <div className={`relative overflow-hidden rounded-2xl border shadow-lg z-10 flex flex-col ${paperBg} ${borderColor}`}>
          ...
          <div className={`px-6 py-5 border-t flex items-center justify-between ${
            ...
            {/* Call to Action */}
            <div className="flex items-center gap-2 text-xs uppercase tracking-[0.15em] font-sans font-bold text-rose-700 dark:text-rose-300">
