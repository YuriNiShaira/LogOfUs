...
const StatsCard: React.FC<StatsCardProps> = ({ icon, label, value }) => {
  ...
  // Fixed rotation - hindi nagbabago sa bawat render
  const randomRotation = useMemo(() => {
    // Use label as seed para consistent ang rotation per card
  ...
  // Mobile: No hover animation, just static
  if (isMobile) {
    return (
      <div
        className={`relative flex flex-col items-center justify-center p-6 pt-10 sm:p-8 sm:pt-12 w-full shadow-[0_5px_15px_rgba(0,0,0,0.08)] dark:shadow-[0_8px_20px_rgba(0,0,0,0.4)] transition-all duration-300 ${cardTheme}`}
        style={{
          borderRadius: '2px',
      ...
      >
        ...
        {/* Value */}
        <p className="text-4xl sm:text-5xl font-bold font-serif tracking-tighter mb-4 text-gray-900 dark:text-white">
